...
# 四种异常文件 (确保路径正确)
# 注意：这里假设每个文件夹下都有 1.csv，如果没有，请改成实际存在的文件名
paths = {
...
try:
    ...
    for key, p in paths.items():
        if os.path.exists(p):
            ...
            print(f"✅ 读取 {key}: 窗口 {windows[key]}")
        else:
            print(f"⚠️ 文件不存在: {p} (将跳过绘图)")
            dfs[key] = None
...
# --- (C) Course Deviation (航道 - 特殊处理：画轨迹) ---
ax = axes[1, 0]
if dfs['course'] is not None:
    s, e = windows['course']
    # 这里我们画 Latitude vs Longitude (轨迹图)
    # 为了显示清楚，我们需要稍微把正常轨迹偏移一点点或者只画对比
...
print("🎉 4合1 最终图已生成: Figure1_FullValidation.pdf")
plt.show()