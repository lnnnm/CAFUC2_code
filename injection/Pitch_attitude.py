...
# ]
# OUTPUT_ROOT = Path(r"C:\Users\DELL\Desktop\RTdetector-main (2)\abnormal_data\Pitch_attitude")
# INPUT_FOLDERS = [
#     "normal_data/C172R",
#     "normal_data/C172S",
#     "normal_data/SR20",
...
PITCH_LOW = -15.0
PITCH_HIGH = 18.0
SEG_RATIO = 0.03         # 约 1.2% 的数据段发生异常
SEG_DUR_SAMPLES = (1, 4)   # 每段持续样本数（相当于 1~4 秒）
PITCH_ADD = (5, 15)        # ±5~15 度变化
...
def choose_segments(df) -> list[tuple[int, int, float]]:
    ...
    segs = []
    for _ in range(n_seg * 2):  # 多尝试几次以避免重叠
        if len(segs) >= n_seg:
            break
    ...
    for fname in tqdm(os.listdir(input_dir), desc=f"processing {input_dir.name}"):
        ...
        try:
            ...
            if PITCH_COL not in df.columns:
                print(f"[WARN] 缺少 Pitch 列，跳过: {fname}")
                continue
...
if meta:
    ...
    print(f"📄 配置信息已保存至: {OUTPUT_ROOT / 'config.json'}")
else:
    print("\n⚠️ 未生成任何异常数据")
